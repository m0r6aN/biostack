...
public sealed class KnowledgeService : IKnowledgeService
{
    ...
    private static KnowledgeEntryResponse MapToResponse(KnowledgeEntry entry)
    {
        return new KnowledgeEntryResponse(
            ...
            entry.DrugInteractions ?? string.Empty,
            entry.OptimizationProtein ?? string.Empty,
            entry.OptimizationCarbs ?? string.Empty,
            entry.OptimizationSupplements ?? string.Empty,
            entry.OptimizationSleep ?? string.Empty,
            entry.OptimizationExercise ?? string.Empty
        );
    }
}
