...
- HTTP JSON handling registers Fido2's credential-type and transport converters ahead of the general API enum converter. This preserves WebAuthn names such as `public-key` and `smart-card`; the general .NET enum-name converter cannot deserialize those browser values. Integration tests submit browser-shaped JSON, perform real synthetic ES256 registration and assertion verification, and check replay, origin, and user-verification rejection. These tests do not establish live Google Password Manager or Windows Hello acceptance.
