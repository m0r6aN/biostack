...
| Browser verification at mobile, tablet, and desktop widths | failed | Executed 2026-08-28 against production at 375x812 / 768x1024 / desktop (`.audit/a11y-tools-2026-08-28.md`): visuals, worded summaries, landmarks, headings, contrast, reduced-motion, and zero console errors all pass; two defects found — unlabeled powder-amount input/unit select (WCAG 1.3.1/4.1.2) and a 16px info trigger (2.5.8) — both fixed on branch `lead/category-language-unify`. | Accessibility/QA owner: re-run the audit against production after the branch deploys, then flip to verified. | Release blocker until the labeled-control fix is live. |
